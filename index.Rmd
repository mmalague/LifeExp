...
In this study, I examine the correlation between health insurance coverage and life expectancy, across counties in the New England states. I hypothesize that communities with less health insurance coverage will have lower average life expectancies. 
...
I have elected to study the relationships between these two variables in New England, a region comprising Connecticut, Maine, Massachusetts, New Hampshire, Rhode Island, and Vermont. This region is relatively socioeconomically diverse, though more racially diverse in some portions/states than others
...
The explanatory variable of interest is statistics on the percentage of a given New England county that does not have health insurance coverage. This explanatory variable is sourced from the American Community Survey, via the Census Bureau. My outcome variable is the life expectancy in the same given county, as measured by the Center for Disease Control and Census data. 
A summary of the dependent variable is below:
...
The following visualization includes a point for each New England county, colored according to its state. 
...
```{r, results='hide', echo = FALSE, results = 'asis'}
...
PERFECT |>
  ggplot(aes(x = `Percent of Population with No Health Insurance Coverage`, y = mean_LE)) +
   geom_smooth(method = "lm", se = TRUE) + geom_hline(aes(yintercept = mean(mean_LE)), color = "black", linetype = 3) + geom_vline(aes(xintercept = mean(`Percent of Population with No Health Insurance Coverage`)), color = "black", linetype = 3) +
  labs(x = "Percent of Population with No Health Insurance Coverage",
       y = "Mean Life Expectancy",
       title = "Percent of Population without Health Insurance -- and Average Life Expectancy",
       subtitle = "Among Counties in the Northeast")
...
PERFECT |>
  ...
  geom_hline(aes(yintercept = mean(mean_LE)), color = "black", linetype = 3) + 
  geom_vline(aes(xintercept = mean(`Percent of Population with No Health Insurance Coverage`)), color = "black", linetype = 3) + 
  labs(x = "Percent of Population with No Health Insurance Coverage",
       y = "Mean Life Expectancy",
       title = "Percent of Population without Health Insurance -- and Average Life Expectancy",
